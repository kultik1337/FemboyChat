-- The admin list draws real avatars, and the app's Avatar component needs the
-- emoji + colour tile as a fallback, not just an image URL.
--
-- NOTE: this version wrote the fallback emoji as a literal escape sequence,
-- which Postgres stores verbatim. It is fixed in the next migration; the broken
-- version is kept here so the history replays exactly as it happened.
drop function if exists public.list_perks(text, integer);

create function public.list_perks(q text default null, lim integer default 60)
returns table (
  uid uuid,
  username text,
  name text,
