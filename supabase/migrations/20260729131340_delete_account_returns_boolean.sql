-- A void return makes success indistinguishable from failure on the client:
-- both arrive as null. Return a boolean so the UI can tell whether the
-- account is really gone before it wipes local data and signs out.
...
-- A dropped function loses its grants, so restore them explicitly.
